    ...
    <div style={{ 
      ...
      <div style={{ flexShrink: 0 }}>
         <div style={{ 
            ...
            <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: '#090d16', padding: '4px' }}>
               <div style={{ 
                 width: '100%', 
                 height: '100%', 
                 borderRadius: '50%', 
      ...
      <div style={{ flex: 1, width: mob ? '100%' : 'auto' }}>
         <div style={{ display: 'flex', flexDirection: mob ? 'column' : 'row', alignItems: 'center', gap: '20px', marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', justifyContent: mob ? 'center' : 'flex-start' }}>
               <h1 style={{ fontSize: '26px', fontWeight: 900, letterSpacing: '-0.02em', margin: 0, color: '#fff', fontFamily: 'Outfit, sans-serif' }}>{content.username}</h1>
            ...
            <div style={{ display: 'flex', gap: '8px', width: mob ? '100%' : 'auto' }}>
               <button 
                  ...
                  style={{ 
                    flex: 1, 
                    padding: '9px 16px', 
                    background: isFollowing ? 'rgba(255, 255, 255, 0.08)' : 'linear-gradient(135deg, #FF9431 0%, #EA580C 100%)', 
                  ...
                  onMouseEnter={(e) => {
                    if (isFollowing) e.currentTarget.style.background = 'rgba(255, 255, 255, 0.15)';
                    else e.currentTarget.style.filter = 'brightness(1.1)';
                  }}
                  onMouseLeave={(e) => {
                    if (isFollowing) e.currentTarget.style.background = 'rgba(255, 255, 255, 0.08)';
         ...
         {!mob && (
            <div style={{ display: 'flex', gap: '40px', marginBottom: '20px', fontSize: '15px', color: '#94a3b8' }}>
               <span><strong style={{ color: '#fff', fontSize: '17px', marginRight: '4px' }}>{postsCount}</strong> posts</span>
               <span><strong style={{ color: '#fff', fontSize: '17px', marginRight: '4px' }}>{followersCount.toLocaleString()}</strong> followers</span>
               <span><strong style={{ color: '#fff', fontSize: '17px', marginRight: '4px' }}>{followingCount}</strong> following</span>
            </div>
         )}
         <div style={{ fontSize: '15px', lineHeight: 1.6 }}>
            <div style={{ fontWeight: 800, fontSize: '17px', marginBottom: '4px', color: '#fff', fontFamily: 'Outfit, sans-serif' }}>{content.displayName}</div>
            ...
            <div style={{ whiteSpace: 'pre-line', color: '#cbd5e1', fontWeight: 500 }}>{content.bio}</div>
            ...
            <div style={{ display: 'flex', flexDirection: mob ? 'column' : 'row', alignItems: mob ? 'center' : 'flex-start', gap: mob ? '12px' : '24px', marginTop: '16px' }}>
               <a 
                 ...
                 style={{ textDecoration: 'none', color: '#38bdf8', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', fontSize: '14px', transition: 'color 0.2s' }}
