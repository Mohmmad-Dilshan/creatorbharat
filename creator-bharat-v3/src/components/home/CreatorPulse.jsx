      ...
      <div style={W()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={item.t} className="au" style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.06)', padding: mob ? '16px 20px' : '24px 32px', borderRadius: 24, boxShadow: '0 20px 40px rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: 20, minWidth: mob ? '100%' : 260, flex: 1 }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: item.c + '10', color: item.c, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }} aria-hidden="true">{item.i}</div>
          ...
          <div style={{ display: 'flex', gap: mob ? 16 : 32, overflow: 'hidden', width: '100%', justifyContent: 'center', flexWrap: 'wrap' }}>
            {spotlightCreators.map((creator) => (
              <div key={creator.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 800, color: '#111' }}>@{creator.handle}</div>
              </div>
            ))}
          </div>
        </div>
