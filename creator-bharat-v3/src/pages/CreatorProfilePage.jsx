    ...
    <div style={{ background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ height: mob ? 200 : 320, background: 'linear-gradient(135deg, #0f172a, #1e293b)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ ...W(1180), padding: '24px', position: 'relative', height: '100%' }}>
           <button onClick={() => go('creators')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: '#fff', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
             ← MARKETPLACE
           </button>
        </div>
      </div>
        ...
        <div style={{ background: '#fff', borderRadius: 32, border: '1px solid rgba(0,0,0,0.04)', padding: mob ? '24px' : '40px', marginTop: -80, position: 'relative', zIndex: 10, boxShadow: '0 30px 60px rgba(0,0,0,0.05)', marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: mob ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 16, marginTop: 40, paddingTop: 32, borderTop: '1px solid rgba(0,0,0,0.04)' }}>
             {[
               ...
               <div key={i} style={{ background: '#f8fafc', borderRadius: 24, padding: '24px', border: '1px solid rgba(0,0,0,0.02)' }}>
                  <div style={{ fontSize: 18, marginBottom: 12, background: st.c + '15', width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{st.i}</div>
                  <div style={{ fontSize: 24, fontWeight: 900, color: '#111' }}>{st.v}</div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', marginTop: 4 }}>{st.l}</div>
               </div>
             ))}
          </div>
        </div>
