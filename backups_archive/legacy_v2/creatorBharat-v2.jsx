...
// CSS
try {
  const s = document.createElement('style'); s.textContent = `@import url('https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,700;9..144,900&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
      ...
      msgs.length <= 1 && React.createElement('div', { style: { padding: '0 12px 10px', display: 'flex', gap: 6, flexWrap: 'wrap', background: '#FAFAF9', flexShrink: 0 } },
        QUICK.map(function (q) {
          return React.createElement('button', {
            key: q, onClick: function () { setInput(q); }, style: {
              ...
              {packages.map(function (pkg) {
                return React.createElement('div', { key: pkg.name, style: { borderRadius: 16, border: '2px solid ' + (pkg.pop ? pkg.color : T.bd), padding: '22px', position: 'relative', background: pkg.pop ? (pkg.color + '06') : '#fff' } },
                  ...
                  React.createElement(Btn, {
                    full: true, style: { background: pkg.pop ? ('linear-gradient(135deg,' + pkg.color + ',' + pkg.color + 'cc)') : 'transparent', border: '2px solid ' + pkg.color, color: pkg.pop ? '#fff' : pkg.color, fontWeight: 700 }, onClick: function () {
                      if (!st.user) { dsp({ t: 'UI', v: { authModal: true, authTab: 'login' } }); return; }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 500 }}>
          <thead>
            <tr>
              ...
              {creators.map(c => {
                const img = c.photo || c.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(c.name)}&background=DC2626&color=fff`; return <th key={c.id} style={{ padding: '12px 16px', textAlign: 'center', borderBottom: `2px solid ${T.bd}` }}>
