...
function DemoModal({onClose}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:999999,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(20px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      ...
      <div style={{width:'100%',maxWidth:380,height:'90vh',maxHeight:840,background:'#fff',borderRadius:40,border:'8px solid #111',boxShadow:'0 40px 80px rgba(0,0,0,0.6), inset 0 0 0 2px rgba(255,255,255,0.2)',position:'relative',display:'flex',flexDirection:'column',overflow:'hidden',animation:'fadeUp 0.3s ease-out'}}>
        ...
        <div style={{flex:1,overflowY:'auto',background:'#F9FAFB',paddingBottom:120}} className="no-scrollbar">
          ...
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',marginTop:-50,position:'relative',zIndex:2}}>
             ...
             <div style={{marginTop:-12,background:'#fff',color:'#10B981',fontSize:11,fontWeight:800,padding:'4px 12px',borderRadius:20,border:'1px solid rgba(16,185,129,0.2)',display:'flex',alignItems:'center',gap:4,boxShadow:'0 4px 10px rgba(16,185,129,0.15)'}}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="#10B981"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg> Verified
             </div>
          </div>
          ...
          <div style={{padding:'20px 24px',textAlign:'center'}}>
            ...
            {/* Links / Services */}
            <h3 style={{fontSize:15,fontWeight:800,textAlign:'left',marginBottom:16,color:'#111',textTransform:'uppercase',letterSpacing:'1px'}}>Services</h3>
            <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:32}}>
               ...
               <div style={{background:'#fff',padding:'16px',borderRadius:16,border:'1px solid rgba(0,0,0,0.05)',display:'flex',alignItems:'center',justifyContent:'space-between',boxShadow:'0 4px 12px rgba(0,0,0,0.02)'}}>
                 <span style={{fontWeight:700,fontSize:14,color:'#111'}}>📱 3x Insta Stories</span>
                 <span style={{fontWeight:800,fontSize:14,color:'#10B981'}}>₹8k</span>
               </div>
            </div>

            {/* Past Campaigns */}
            <h3 style={{fontSize:15,fontWeight:800,textAlign:'left',marginBottom:16,color:'#111',textTransform:'uppercase',letterSpacing:'1px'}}>Brands I've Worked With</h3>
            ...
            {/* Gallery */}
            <h3 style={{fontSize:15,fontWeight:800,textAlign:'left',marginBottom:16,color:'#111',textTransform:'uppercase',letterSpacing:'1px'}}>Recent Work</h3>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:40}}>
               <img src="https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=400&q=80" style={{width:'100%',height:180,objectFit:'cover',borderRadius:16}} alt=""/>
               <img src="https://images.unsplash.com/photo-1503220317375-aaad61436b1b?w=400&q=80" style={{width:'100%',height:180,objectFit:'cover',borderRadius:16}} alt=""/>
               <img src="https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=400&q=80" style={{width:'100%',height:180,objectFit:'cover',borderRadius:16}} alt=""/>
               <img src="https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=400&q=80" style={{width:'100%',height:180,objectFit:'cover',borderRadius:16}} alt=""/>
            </div>
...
const startIdx = code.indexOf('// DEMO CREATOR PORTFOLIO MODAL');
const endIdx = code.indexOf('function HomePage(){');

if (startIdx !== -1 && endIdx !== -1) {
  code = code.substring(0, startIdx) + demoModalCode + '\n' + code.substring(endIdx);
  fs.writeFileSync(file, code);
  console.log('DemoModal replaced with ultra-premium z-index 999999 version');
} else {
  console.log('Could not find existing DemoModal');
}
